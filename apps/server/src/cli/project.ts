...
const findActiveProjectTarget = Effect.fn("findActiveProjectTarget")(function* (input: {
  ...
  // The shell snapshot already excludes deleted projects, so every project
  // here is active.
...
// Liveness probe for `tryResolveLiveProjectExecutionMode` below. It asks the
// same question as `fetchLiveOrchestrationSnapshot` — "is a server alive at
// this origin?" — but throws the response away, and funnels failures through
// the same `projectCommandErrorFromLiveServerRequest`, so
