...
export const make = Effect.gen(function* () {
  ...
  const isRegisteredWorktreePath = Effect.fn("WorktreeProvisioner.isRegisteredWorktreePath")(
    function* (cwd: string, targetPath: string) {
      ...
      return result.stdout.split(/\r?\n/).some((line) => {
        ...
        return path.resolve(line.slice("worktree ".length)) === normalizedTarget;
      });
    },
  );
  ...
  const refuseExistingTarget = Effect.fn("WorktreeProvisioner.refuseExistingTarget")(function* (
    ...
    if (yield* isRegisteredWorktreePath(cwd, targetPath)) {
  ...
  const provision: WorktreeProvisioner["Service"]["provision"] = Effect.fn(
    ...
    if (input.refuseExisting === true && input.path !== null) {
  ...
  const stopSessionAttempt = Effect.fn("WorktreeProvisioner.stopSession")(function* (
    ...
    yield* startup.enqueueCommand(orchestrationEngine.dispatch(stopCommand));
