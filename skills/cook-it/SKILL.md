...
If BLOCK, send a follow-up to the implementer agent (via `SendMessage` to its agent id) describing exactly what to fix. Re-run only the reviewer(s) whose scope the fix touched. Don't get stuck — if a second BLOCK round produces conflicting feedback, surface to the user.
...
## Run it as a workflow when possible
...
If the `Workflow` tool isn't available, fall back to dispatching the agents manually as described above.
