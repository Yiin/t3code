...
5. Close out: `bd close @CHILD@`, then `bd note @EPIC@ "@CHILD@ done — <one clause, mentioning any dependency added and which repos gained commits>. DECISION: <one line each, only if it changes remaining children>. GOTCHA: <one line each, only if siblings will hit it>."` Cap the whole note at 400 characters. The coordinator folds DECISION:/GOTCHA: lines into the epic's Context & architecture after landing — you must never edit the epic body yourself.
