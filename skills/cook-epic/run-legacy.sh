#!/usr/bin/env bash
# cook-epic run-legacy — the deprecated Bash coordinator, kept for one release.
#
# Deprecated: run.sh now execs `t3 epic cook` (the shared TypeScript core) by
# default. This coordinator stays reachable through `run.sh` with
# COOKEPIC_CORE=0 (or COOKEPIC_CORE=legacy) for one documented release, then
# t3code-06s.42 removes it. Do not add features here.
#
...
#
# Usage: run-legacy.sh <run-dir>   # launch from the project root
#
...
RUN_DIR="${1:?usage: run-legacy.sh <run-dir>}"
mkdir -p "$RUN_DIR"
