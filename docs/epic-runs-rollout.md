...
| Dropped knob                                                                                                                                                                                                                                              | Reason                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `COOKEPIC_WORKERS`, `COOKEPIC_SIBLINGS`                                                                                                                                                                                                                   | Restored by t3code-06s.43 on 2026-08-08: the terminal cook takes its execution shape from config again — `parallel.workers` above 1 drives `runParallelEpicLoop` through file-backed terminal ports, and `parallel.siblings` mirrors per-worker layouts. No longer dropped.                                                             |
...
| `COOKEPIC_IDLE_THRESHOLD`, `COOKEPIC_INSPECTOR_TIMEOUT`, `COOKEPIC_INSPECT_RETRY_DELAY`, `COOKEPIC_INSPECT_MIN_DELAY`, `COOKEPIC_INSPECT_MAX_DELAY`, `COOKEPIC_INSPECTOR_CMD`                                                                             | The env knobs are dropped; the settings are not. `workerSupervision.ts` drives the liveness machine in the parallel loop, and the inspector runs on the `prime`, `claude` and `ccx` harnesses (`adapters/AgentInspector.ts`). Set them through `supervision.*` in `.t3code/epic-run.json`. `COOKEPIC_INSPECTOR_CMD` has no replacement. |
...
| `COOKEPIC_REPO_PROBE_INTERVAL`, `COOKEPIC_REPO_PROBE_TIMEOUT`                                                                                                                                                                                             | The repository probe runs, on the machine's own defaults in `DEFAULT_WORKER_LIVENESS_CONFIG` (60s interval, 2s timeout). Only the env knobs are dropped.                                                                                                                                                                                |
